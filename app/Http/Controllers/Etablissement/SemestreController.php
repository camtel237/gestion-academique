...
{
    // Année active par défaut si rien n'est explicitement choisi
    $anneeActive = AnneeAcademique::where('est_active', true)->first();

    $anneeFiltre = $request->filled('annee_academique_id')
    ...
    if ($anneeFiltre) {
        $query->where('annee_academique_id', $anneeFiltre);
    }
    ...
    $semestres = $query->orderBy('libelle')->paginate(10)->withQueryString();

    $anneesAcademiques = AnneeAcademique::orderBy('libelle', 'desc')->get();

    $niveaux = Niveau::where('est_actif', true)
        ->where('annee_academique_id', $anneeFiltre)
        ->with(['specialite'])
        ->orderBy('libelle')
        ->get()
        ->map(function ($niveau) {
    ...
    return view('etablissement.semestres.index', compact('semestres', 'anneesAcademiques', 'niveaux', 'anneeFiltre'));
}

...
class PersonnelController extends Controller
{
    ...
    public function store(StorePersonnelRequest $request)
    {
        ...
        $dernier = \App\Models\Etablissement\Personnel::count();
        $data['matricule'] = 'EMP' . str_pad($dernier + 1, 3, '0', STR_PAD_LEFT);
        while (\App\Models\Etablissement\Personnel::where('matricule', $data['matricule'])->exists()) {
            $dernier++;
            $data['matricule'] = 'EMP' . str_pad($dernier + 1, 3, '0', STR_PAD_LEFT);
        }

        \App\Models\Etablissement\Personnel::create($data);

        return redirect()->route('personnels.index')
            ->with('success', 'Employé ajouté avec succès. Matricule : ' . $data['matricule']);
    }

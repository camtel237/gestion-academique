...
@section('content')
<form method="GET" action="{{ route('inscriptions.index') }}" id="inscriptionsFilterForm">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        <div class="flex flex-wrap gap-2 flex-1">
            ...
            <select name="niveau_id" id="niveau_filter" onchange="this.form.submit()"
                    class="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-200 outline-none transition"
                    {{ $niveaux->isEmpty() ? 'disabled' : '' }}>
                <option value="">{{ $niveaux->isEmpty() ? "Choisir une spécialité d'abord" : 'Tous les niveaux' }}</option>
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        ...
                        <form action="{{ route('inscriptions.destroy', $inscription) }}" method="POST" class="inline">
                            ...
                            @method('DELETE')
                            <button type="button"
                                onclick="askDeleteConfirm(this.closest('form'), 'Supprimer l\'inscription de {{ $inscription->etudiant->prenom }} {{ $inscription->etudiant->nom }} ?')"
                                class="w-8 h-8 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 inline-flex items-center justify-center ml-1 transition"
                                title="Supprimer">

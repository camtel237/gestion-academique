{{-- resources/views/effets/releves/pdf.blade.php --}}
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page {
            width: 210mm;
            height: 297mm;
            background-image: url('{{ public_path('images/effets/certificat-fond.png') }}');
...
<body>
    <div class="page">
        <div class="content">
            ...
            <div class="entete">
                <img src="{{ public_path('images/effets/certificat-entete.png') }}">
            </div>
            ...
            <div class="numero">
                N° {{ str_pad($numero, 6, '0', STR_PAD_LEFT) }}-{{ config('etablissement.code') }}/{{ date('Y') }}/{{ config('etablissement.code') }}/DA/DASR/SCO/CISI
            </div>
            ...
            <table class="champs">
                <tr>
                    <td width="62%">
                        ...
                        <span class="val">{{ mb_strtoupper($inscription->etudiant->nom . ' ' . $inscription->etudiant->prenom, 'UTF-8') }}</span>
                ...
                <tr>
                    <td>
                        <div class="birth-block" style="margin-right: 15px;">
                            ...
                            <span class="val">{{ $inscription->etudiant->date_naissance ? \Carbon\Carbon::parse($inscription->etudiant->date_naissance)->format('d / m / Y') : '-' }}</span>
                            <span class="sub-en">Born on the</span>
                        </div>
                        <div class="birth-block">
                            <span class="lbl">à :</span>
                            <span class="val">{{ $inscription->etudiant->lieu_naissance ?? '-' }}</span>
                ...
                <tr>
                    <td>
                        ...
                        <span class="val">{{ mb_strtoupper($inscription->departement->libelle ?? '-', 'UTF-8') }}</span>
...
<span class="val">{{ mb_strtoupper($inscription->specialite->libelle ?? '-', 'UTF-8') }}</span>                        <span class="en">Speciality / Option</span>
                    </td>
                    <td>
                        <span class="lbl">Niveau :</span>
                        <span class="val">{{ $inscription->niveau->libelle ?? '-' }}</span>
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <span class="val">{{ $inscription->anneeAcademique->libelle ?? '-' }}</span>
            ...
            <table class="notes">
                ...
                <tbody>
                    @foreach($ues as $ue)
                        <tr class="ue-row">
                            <td colspan="3">{{ $ue->code }} : {{ $ue->libelle }}</td>
                            <td class="center">{{ $ue->moyenne !== null ? number_format($ue->moyenne, 2) : '-' }}</td>
                            <td class="center">{{ $ue->credit }}</td>
                            <td class="center">{{ $ue->grade }}</td>
                            <td class="center">{{ $ue->decision }}</td>
                        </tr>
                        @foreach($ue->matieres as $matiere)
                            <tr>
                                ...
                                <td class="center">{{ $matiere->moyenne !== null ? number_format($matiere->moyenne, 2) : '-' }}</td>
                                <td class="center">{{ $matiere->credit }}</td>
                                <td class="center">{{ $matiere->grade }}</td>
                                <td class="center">{{ $matiere->decision }}</td>
                            </tr>
                        @endforeach
                    @endforeach
                    ...
                    <tr class="synthese-row">
                        ...
                        <td>{{ $synthese->ue_acquis }}</td>
                        <td>{{ $synthese->ec_acquis }}</td>
                        <td rowspan="2" style="vertical-align: middle;">{{ $synthese->credit_acquis }}</td>
                        <td rowspan="2" style="vertical-align: middle;">{{ $synthese->moyenne !== null ? number_format($synthese->moyenne, 2) : '-' }}</td>
                        <td rowspan="2" style="vertical-align: middle;">{{ $synthese->grade }}</td>
                        <td rowspan="2" style="vertical-align: middle;">{{ $synthese->mention }}</td>
                    </tr>
                    <tr class="synthese-row">
                        <td style="text-align:left;">Taux</td>
                        <td>{{ number_format($synthese->taux_ue, 2) }}%</td>
                        <td>{{ number_format($synthese->taux_ec, 2) }}%</td>
                    </tr>
                    <tr class="caption">
            ...
            <div class="qr-wrap">
                <img src="{{ public_path('images/effets/qr-code.png') }}">
            </div>

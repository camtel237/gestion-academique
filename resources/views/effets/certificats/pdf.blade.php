{{-- resources/views/effets/certificats/pdf.blade.php --}}
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="page">
        <div class="content">
            {{-- En-tête officiel bilingue (logo + ministère) --}}
            <div class="entete">
            ...
            <div class="photo-wrap">
                @if($inscription->etudiant->photo && file_exists(public_path('storage/' . $inscription->etudiant->photo)))
                    <img src="{{ public_path('storage/' . $inscription->etudiant->photo) }}">
                @endif
            </div>
            ...
            <div class="numero">
                N° {{ str_pad($numero, 5, '0', STR_PAD_LEFT) }}-{{ config('etablissement.code') }}/{{ date('Y') }}/{{ config('etablissement.code') }}/DA/DASR/CD/SCO/CISI
            </div>

            {{-- Paragraphe d'attestation (image fournie, fond transparent) --}}
            <div class="intro-img">
                <img src="{{ public_path('images/effets/certificat-intro.png') }}">
            </div>
            ...
            <table class="fields">
                <tr>
                    <td width="60%">
                        ...
                        {{ strtoupper($inscription->etudiant->nom . ' ' . $inscription->etudiant->prenom) }}
                ...
                <tr>
                    <td width="50%">
                        ...
                        <span class="value">{{ $inscription->etudiant->date_naissance ? \Carbon\Carbon::parse($inscription->etudiant->date_naissance)->format('Y-m-d') : '-' }}</span>
                        <span class="en">Born on the</span>
                    </td>
                    <td width="50%">
                        <span class="lbl">à :</span> 
                        <span class="value">{{ $inscription->etudiant->lieu_naissance ?? '-' }}</span>
                        <span class="en">at</span>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <span class="lbl">Matricule :</span> {{ $inscription->etudiant->matricule }}
            ...
            <table class="fields" style="margin-top: 16px;">
                <tr>
                    <td width="60%">
                        <span class="lbl">Département :</span> {{ $inscription->departement->libelle ?? '-' }}
                        <span class="en">Department</span>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <span class="lbl">Spécialité / Option :</span> {{ $inscription->specialite->libelle ?? '-' }}
                        <span class="en">Speciality / Option</span>
                    </td>
                    <td width="40%">
                        <span class="lbl">Niveau :</span> {{ $inscription->niveau->libelle ?? '-' }}
                        <span class="en">level</span>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" style="padding-bottom:0;">
                        <span class="lbl">Année académique :</span> {{ $inscription->anneeAcademique->libelle ?? '-' }}
                        <span class="en">Academic year</span>
                    </td>
                </tr>
            </table>

            {{-- Formule de clôture + signature (image fournie, exactement comme l'original) --}}
            <div class="footer-img">
                <img src="{{ public_path('images/effets/certificat-footer.png') }}">
            </div>

            {{-- QR code de vérification, en bas à gauche comme dans l'original --}}
            <div class="qr-wrap">

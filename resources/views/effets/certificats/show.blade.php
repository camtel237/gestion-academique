{{-- resources/views/effets/certificats/show.blade.php --}}
@extends('layouts.app')

@section('title', 'Aperçu du certificat - EduManager')
...
<div class="max-w-3xl mx-auto">
    <div class="flex justify-end gap-2 mb-4">
       <a href="{{ route('effectifs.index', ['specialite_id' => request('specialite_id'), 'niveau_id' => request('niveau_id')]) }}"
        class="px-4 py-2.5 border border-slate-200 rounded-xl text-sm hover:bg-slate-50 transition bg-white">
            <i class="fa-solid fa-arrow-left mr-1"></i> Retour
        </a>
        <a href="{{ route('certificats.download', $inscription->id) }}"
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden" style="height: 80vh;">
        <iframe src="{{ route('certificats.preview', $inscription->id) }}" class="w-full h-full" style="border:none;"></iframe>
    </div>
</div>
@endsection
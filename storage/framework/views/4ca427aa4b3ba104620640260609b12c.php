<?php $__env->startSection('title', 'Personnel - EduManager'); ?>
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
    <div class="relative max-w-sm flex-1">
        ...
        <form method="GET" action="<?php echo e(route('personnels.index')); ?>" class="inline">
    ...
    <div class="overflow-x-auto scrollbar-thin">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                <?php $__empty_1 = true; $__currentLoopData = $personnels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $personnel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-slate-50 transition">
                    <td class="px-4 py-3 font-bold text-brand-700"><?php echo e($personnel->matricule); ?></td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-full <?php echo e($personnel->sexe === 'F' ? 'bg-pink-500' : 'bg-brand-600'); ?> text-white flex items-center justify-center text-xs font-semibold">
                                <?php echo e(strtoupper(substr($personnel->prenom, 0, 1))); ?><?php echo e(strtoupper(substr($personnel->nom, 0, 1))); ?>

                            </div>
                            <span class="font-medium"><?php echo e($personnel->prenom); ?> <?php echo e($personnel->nom); ?></span>
                        </div>
                    </td>
                    <td class="px-4 py-3"><?php echo e($personnel->fonction); ?></td>
                    <td class="px-4 py-3"><?php echo e($personnel->email ?? '-'); ?></td>
                    <td class="px-4 py-3"><?php echo e($personnel->telephone ?? '-'); ?></td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3">
                        <span class="px-2 py-1 rounded-full text-xs font-semibold <?php echo e($personnel->est_actif ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'); ?>">
                            <?php echo e($personnel->est_actif ? 'Actif' : 'Inactif'); ?>

                        </span>
                    </td>
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        <a href="<?php echo e(route('personnels.show', $personnel)); ?>"
                        ...
                        <form action="<?php echo e(route('personnels.toggle-status', $personnel)); ?>" method="POST" class="inline">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('PATCH'); ?>
                            <button type="submit"
                                    class="w-8 h-8 rounded-lg <?php echo e($personnel->est_actif ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-green-100 text-green-600 hover:bg-green-200'); ?> inline-flex items-center justify-center ml-1 transition"
                                    title="<?php echo e($personnel->est_actif ? 'Désactiver' : 'Activer'); ?>">
                                <i class="fa-solid <?php echo e($personnel->est_actif ? 'fa-pause' : 'fa-play'); ?> text-xs"></i>
                            </button>
                        </form>
                        <form action="<?php echo e(route('personnels.destroy', $personnel)); ?>" method="POST" class="inline">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('DELETE'); ?>
                            <button type="button" onclick="askDeleteConfirm(this.closest('form'), 'Confirmer la suppression de l\'employé <?php echo e($personnel->prenom); ?> <?php echo e($personnel->nom); ?> ?')"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\gestion-academique\resources\views/etablissement/personnels/index.blade.php ENDPATH**/ ?>
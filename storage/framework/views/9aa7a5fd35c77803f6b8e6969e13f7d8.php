<?php $__env->startSection('title', 'Liste des inscriptions - EduManager'); ?>

<?php
    $pageTitle = 'Liste des inscriptions';
    $pageSub = 'Gestion des inscriptions des étudiants';
?>

<?php $__env->startSection('content'); ?>
<form method="GET" action="<?php echo e(route('inscriptions.index')); ?>" id="inscriptionsFilterForm">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        <div class="flex flex-wrap gap-2 flex-1">
            <div class="relative max-w-sm flex-1 min-w-[200px]">
                <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
                <input type="text" name="search" placeholder="Rechercher une inscription..."
                       value="<?php echo e(request('search')); ?>"
                       class="w-full pl-10 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-200 outline-none transition"/>
            </div>

            <select name="specialite_id" id="specialite_filter" onchange="this.form.submit()"
                    class="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-200 outline-none transition">
                <option value="">Toutes les spécialités</option>
                <?php $__currentLoopData = $specialites; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $specialite): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($specialite->id); ?>" <?php echo e(request('specialite_id') == $specialite->id ? 'selected' : ''); ?>>
                        <?php echo e($specialite->libelle); ?>

                    </option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>

            <select name="niveau_id" id="niveau_filter" onchange="this.form.submit()"
                    class="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-200 outline-none transition"
                    <?php echo e($niveaux->isEmpty() ? 'disabled' : ''); ?>>
                <option value=""><?php echo e($niveaux->isEmpty() ? "Choisir une spécialité d'abord" : 'Tous les niveaux'); ?></option>
                <?php $__currentLoopData = $niveaux; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $niveau): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($niveau->id); ?>" <?php echo e(request('niveau_id') == $niveau->id ? 'selected' : ''); ?>>
                        <?php echo e($niveau->libelle); ?>

                    </option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
        <div class="flex gap-2 flex-wrap">
            <button type="button" onclick="toast('Export Excel lancé', 'info')"
                    class="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm hover:bg-slate-50 transition">
                <i class="fa-solid fa-file-excel text-green-600"></i> Excel
            </button>
            <a href="<?php echo e(route('inscriptions.create')); ?>"
               class="px-4 py-2.5 grad-blue text-white rounded-xl text-sm font-semibold shadow hover:opacity-95 transition">
                <i class="fa-solid fa-plus mr-1"></i> Nouvelle inscription
            </a>
        </div>
    </div>
</form>


<div class="bg-white rounded-2xl border border-slate-100 overflow-hidden">
    <div class="overflow-x-auto scrollbar-thin">
        <table class="w-full text-sm">
            <thead class="bg-slate-50 text-slate-600 text-xs uppercase tracking-wider">
                <tr>
                    <th class="px-4 py-3 text-left font-semibold">#</th>
                    <th class="px-4 py-3 text-left font-semibold">Étudiant</th>
                    <th class="px-4 py-3 text-left font-semibold">Année</th>
                    <th class="px-4 py-3 text-left font-semibold">Département</th>
                    <th class="px-4 py-3 text-left font-semibold">Spécialité</th>
                    <th class="px-4 py-3 text-left font-semibold">Niveau</th>
                    <th class="px-4 py-3 text-left font-semibold">Statut</th>
                    <th class="px-4 py-3 text-right font-semibold">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                <?php $__empty_1 = true; $__currentLoopData = $inscriptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $inscription): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-slate-50 transition">
                    <td class="px-4 py-3">#<?php echo e($inscription->id); ?></td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-full <?php echo e($inscription->etudiant->sexe === 'F' ? 'bg-pink-500' : 'bg-brand-600'); ?> text-white flex items-center justify-center text-xs font-semibold">
                                <?php echo e(strtoupper(substr($inscription->etudiant->prenom, 0, 1))); ?><?php echo e(strtoupper(substr($inscription->etudiant->nom, 0, 1))); ?>

                            </div>
                            <span><?php echo e($inscription->etudiant->nom_complet); ?></span>
                        </div>
                    </td>
                    <td class="px-4 py-3"><?php echo e($inscription->anneeAcademique->libelle ?? '-'); ?></td>
                    <td class="px-4 py-3"><?php echo e($inscription->departement->libelle ?? '-'); ?></td>
                    <td class="px-4 py-3"><?php echo e($inscription->specialite->libelle ?? '-'); ?></td>
                    <td class="px-4 py-3"><?php echo e($inscription->niveau->libelle ?? '-'); ?></td>
                    <td class="px-4 py-3">
                        <span class="px-2 py-1 rounded-full text-xs font-semibold 
                            <?php echo e($inscription->statut === 'validee' ? 'bg-green-100 text-green-700' : 
                               ($inscription->statut === 'annulee' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700')); ?>">
                            <?php echo e(ucfirst($inscription->statut)); ?>

                        </span>
                    </td>
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        <a href="<?php echo e(route('inscriptions.show', $inscription)); ?>"
                           class="w-8 h-8 rounded-lg bg-sky-100 text-sky-600 hover:bg-sky-200 inline-flex items-center justify-center ml-1 transition"
                           title="Voir">
                            <i class="fa-solid fa-eye text-xs"></i>
                        </a>
                        <a href="<?php echo e(route('inscriptions.edit', $inscription)); ?>"
                           class="w-8 h-8 rounded-lg bg-amber-100 text-amber-600 hover:bg-amber-200 inline-flex items-center justify-center ml-1 transition"
                           title="Modifier">
                            <i class="fa-solid fa-pen text-xs"></i>
                        </a>
                        <?php if($inscription->statut === 'en_attente'): ?>
                            <form action="<?php echo e(route('inscriptions.valider', $inscription)); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('PATCH'); ?>
                                <button type="submit"
                                        class="w-8 h-8 rounded-lg bg-green-100 text-green-600 hover:bg-green-200 inline-flex items-center justify-center ml-1 transition"
                                        title="Valider">
                                    <i class="fa-solid fa-check text-xs"></i>
                                </button>
                            </form>
                            <form action="<?php echo e(route('inscriptions.annuler', $inscription)); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('PATCH'); ?>
                                <button type="submit"
                                        class="w-8 h-8 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 inline-flex items-center justify-center ml-1 transition"
                                        title="Annuler">
                                    <i class="fa-solid fa-xmark text-xs"></i>
                                </button>
                            </form>
                        <?php endif; ?>
                        <form action="<?php echo e(route('inscriptions.destroy', $inscription)); ?>" method="POST" class="inline">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('DELETE'); ?>
                            <button type="button"
                                onclick="askDeleteConfirm(this.closest('form'), 'Supprimer l\'inscription de <?php echo e($inscription->etudiant->prenom); ?> <?php echo e($inscription->etudiant->nom); ?> ?')"
                                class="w-8 h-8 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 inline-flex items-center justify-center ml-1 transition"
                                title="Supprimer">
                                <i class="fa-solid fa-trash text-xs"></i>
                            </button>   
                        </form>
                    </td>
                </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                <tr>
                    <td colspan="8" class="text-center py-8 text-slate-500">
                        <i class="fa-solid fa-file-signature text-4xl text-slate-300 mb-2 block"></i>
                        Aucune inscription trouvée
                    </td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
</div>

<div class="mt-4"><?php echo e($inscriptions->links()); ?></div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\gestion-academique\resources\views/inscriptions/index.blade.php ENDPATH**/ ?>